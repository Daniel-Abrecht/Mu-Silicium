...
// TODO: I should probably use ACPI instead
...
#define HW_CONTROLLER_DP_STR "\x05"
...
#define DP_CONTROLLER_1 \
  HARDWARE_DEVICE_PATH_STR HW_CONTROLLER_DP_STR "\x08\x00" "\x01\x00\x00\x00" // Connector 1
...
EFI_STATUS EFIAPI Init(
  ...
  Status |= gBS->InstallMultipleProtocolInterfaces(&(EFI_HANDLE){0},
    &gEfiDevicePathProtocolGuid, DP_GLINK_REMOTE_LPASS DP_END, NULL);
  Status |= gBS->InstallMultipleProtocolInterfaces(&(EFI_HANDLE){0},
    &gEfiDevicePathProtocolGuid, DP_GLINK_REMOTE_LPASS DP_GLINK_CHANNEL_ADSP_APPS DP_END, NULL);
  Status |= gBS->InstallMultipleProtocolInterfaces(&(EFI_HANDLE){0},
    &gEfiDevicePathProtocolGuid, DP_GLINK_REMOTE_LPASS DP_GLINK_CHANNEL_ADSP_APPS DP_GLINK_UCSI DP_END, NULL);
  
  Status |= gBS->InstallMultipleProtocolInterfaces(&(EFI_HANDLE){0},
    &gEfiDevicePathProtocolGuid, DP_USB_REDRIVER DP_END, NULL);
  
  {
    static const void*const dependent_devices[] = {
      DP_GLINK_REMOTE_LPASS DP_GLINK_CHANNEL_ADSP_APPS DP_GLINK_UCSI DP_CONTROLLER_1 DP_END,
      DP_USB_REDRIVER DP_END,
      NULL
    };
    Status |= gBS->InstallMultipleProtocolInterfaces(&(EFI_HANDLE){0},
      &gEfiDevicePathProtocolGuid, DP_DWC3_USB_CONTROLLER DP_END,
      &gEfiDependentDevices, dependent_devices,
      NULL);
  }
